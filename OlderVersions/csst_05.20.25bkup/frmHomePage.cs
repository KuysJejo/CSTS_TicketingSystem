...
namespace TicketingSystem
{
    public partial class frmHomePage : Form
    {
        ...
        private void tmrCountTickets_Tick(object sender, EventArgs e)
        {
            Ticket.CountTickets(this);
        }

        private void cmsOpenItem_Click(object sender, EventArgs e)
        {
            if (dgvTickets.RowCount != 0)
            {
                //set the curremt selection
                ...
                if (dgvTickets.SelectedRows.Count > 0 && Ticket.selectedRowIndex > -1)
                {
                    Ticket.CurrentTicketID = dgvTickets.SelectedCells[0].Value.ToString();
                ...
                else if (Ticket.selectedRowIndex<0){
                    MessageBox.Show("Select first an item to open.","Invalid Action",MessageBoxButtons.OK,MessageBoxIcon.None);
                }
            }
        }
